...
{
    // 1. 안전한 문자열 처리를 위해 복사본 생성 및 널 종료 문자 처리
    char msg[64];
    ...
    if (!str_id || !str_type || !str_val) return;
            ...
            if (err == ERR_OK) {
                ...
                while (1) {
                    ...
                    if (recv_err == ERR_OK) {
                        ...
                        // ★ 중요: 서버로부터 데이터가 오지 않아 발생한 타임아웃입니다.
                        // 연결은 살아있는 상태이므로 아무것도 하지 않고 다시 대기합니다.
                        continue; 
                    } 
                    else {
                        // ERR_CLSD (연결 종료) 등 진짜 에러가 발생하면 루프를 나갑니다.
